...
export default function TTSPlayer({ voiceName, text, progress, className = '' }: TTSPlayerProps) {
  const [isPlaying, setIsPlaying] = useState(false)
  // progress is a 0–1 fraction of the waveform; clamp then quantize to bars.
  const playedCount = Math.round(Math.max(0, Math.min(1, progress)) * WAVE_BARS.length)
