...
def _validate_path(path: Path, path_str: str, path_type: str = "file") -> Optional[Dict[str, Any]]:
...
def _validate_file_path(path: Path, file_path: str) -> Optional[Dict[str, Any]]:
    return _validate_path(path, file_path, "file")


def _validate_dir_path(path: Path, dir_path: str) -> Optional[Dict[str, Any]]:
    return _validate_path(path, dir_path, "directory")


def _validate_line_range(start: int, end: int, total: int, start_line: Optional[int]) -> Optional[Dict[str, Any]]:
...
def _build_file_response(path: Path, content: str, lines: List[str], start_line: Optional[int] = None, end_line: Optional[int] = None) -> Dict[str, Any]:
    response = {
        "success": True,
...
def read_file_impl(file_path: str, encoding: str = "utf-8", start_line: Optional[int] = None, end_line: Optional[int] = None) -> Dict[str, Any]:
...
def _format_file_info(item: Path, base_path: Optional[Path] = None) -> Dict[str, Any]:
    stat = item.stat()
    return {
...
def _list_non_recursive(path: Path, pattern: Optional[str]) -> tuple[List[Dict[str, Any]], List[str]]:
    files = []
    directories = []
...
def list_directory_impl(directory_path: str, recursive: bool = False, pattern: Optional[str] = None) -> Dict[str, Any]:
...
def _compile_search_regex(pattern: str, case_sensitive: bool, is_regex: bool) -> Optional[re.Pattern]:
    if not is_regex:
        return None
...
def _line_matches_pattern(line: str, pattern: str, regex: Optional[re.Pattern], case_sensitive: bool) -> bool:
    if regex:
...
def _search_file(file_path: Path, base_path: Path, pattern: str, regex: Optional[re.Pattern], case_sensitive: bool) -> List[Dict[str, Any]]:
    matches = []
    try:
...
def _get_line_count(path: Path) -> Optional[int]:
    if not path.is_file():
        return None
...
def _build_match_info(line_num: int, line: str, lines: List[str], show_context: bool, context_lines: int) -> Dict[str, Any]:
    match_info = {
        "line": line_num,
        "content": line.rstrip()
    }
...
def _count_pattern_in_line(line: str, pattern: str, regex: Optional[re.Pattern], case_sensitive: bool) -> int:
    if regex:
...
def count_in_file_impl(
    ...
    try:
        ...
        lines_with_matches, total_count, _ = _process_file_lines(
            path, pattern, regex, case_sensitive, 'count'
        )
