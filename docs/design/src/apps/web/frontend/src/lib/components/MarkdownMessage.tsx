...
export function MarkdownMessage({
  ...
  const components: Components = {
    ...
    p({ children }) {
      return (
        <p className="mb-4 last:mb-0 leading-relaxed text-base">{children}</p>
      );
    },
    ul({ children }) {
      return (
    ...
    ol({ children }) {
      return (
        <ol className="list-decimal list-inside mb-4 space-y-2 text-base">
          {children}
        </ol>
      );
    },
    ...
    th({ children }) {
      return (
        <th className="px-4 py-2 text-left text-sm font-semibold text-foreground/80">
          {children}
        </th>
      );
    },
