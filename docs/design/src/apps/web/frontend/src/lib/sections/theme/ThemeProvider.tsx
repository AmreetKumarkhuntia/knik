...
export function ThemeProvider({ children }: { children: ReactNode }) {
  const [mode, setModeState] = useState<ThemeMode>(() => {
    try {
      return (
        (localStorage.getItem("knik-theme-mode") as ThemeMode) ?? DEFAULT_MODE
      );
    } catch {
      return DEFAULT_MODE;
    }
  ...
  const [accentName, setAccentState] = useState<ThemeName>(() => {
    try {
      return (
        (localStorage.getItem("knik-theme-accent") as ThemeName) ??
        DEFAULT_THEME
      );
    } catch {
  ...
  const theme: Theme = useMemo(
    () => ({
      ...
      colors:
        mode === "dark"
          ? themePresets[accentName]
          : lightThemePresets[accentName],
    }),
    [mode, accentName],
  );
  ...
  return (
    <ThemeContext.Provider
      value={{ theme, mode, accentName, setMode, setAccent }}
    >
      {children}
    </ThemeContext.Provider>
  );
