...
Query AI (Vertex AI / Gemini) and speak th    # Initialize AI client
    try:
        ai_client = AIClient()
        ...
        ai_client = MockAIClient()se using TTS.
"""
...
from src.lib import AIClient, AudioProcessor, Config, KokoroVoiceModel, MockAIClient
...
def demo_basic_ai_query():
    ...
    try:
        ai_client = AIClient(
            provider="vertex", project_id="dev-ai-gamma", location="us-east5", model_name="gemini-2.5-flash"
        )
        if not ai_client.is_configured():
...
def demo_ai_context_conversation():
    ...
    for question in questions:
        ...
        # Query with context
        print("AI: ", end="", flush=True)
        response = ai_client.query(question, context=context if context else None, max_tokens=200, temperature=0.7)
        print(response)
...
def demo_ai_with_system_instruction():
    ...
    questions = ["What is the weather like today?", "How do I make coffee?", "Tell me a fun fact."]
    ...
    for question in questions:
        ...
        response = ai_client.query(question, max_tokens=150, temperature=0.9, system_instruction=system_instruction)
